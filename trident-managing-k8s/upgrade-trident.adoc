...
`tridentctl` è il metodo convenzionale di installazione Astra Trident e offre il maggior numero di opzioni per chi necessita di personalizzazioni complesse. Per maggiori dettagli, consulta link:../trident-get-started/kubernetes-deploy.html#choose-your-installation-method["Scegliere il metodo di installazione desiderato"].

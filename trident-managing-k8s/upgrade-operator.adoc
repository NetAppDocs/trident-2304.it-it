...
In genere, dovresti aggiornare Astra Trident usando lo stesso metodo che hai usato per installarlo la prima volta. Consulta link:upgrade-trident.html#upgrade-trident.html#step-3-select-an-upgrade-method["Selezionare un metodo di aggiornamento"] prima di provare ad aggiornare con l'operatore Trident.
...
NOTE: Se lo stato mostra `Updating`, assicurati di risolverlo prima di procedere. Per un elenco dei possibili valori di stato, consulta link:../trident-get-started/kubernetes-deploy-operator.html["qui"^].
...
. (Opzionale) Se è necessario modificare i parametri di installazione, aggiorna la specifica  `TridentProvisioner`. Ciò può includere modifiche come la variazione di: i valori per  `tridentImage`,  `autosupportImage`, repository di immagini privato e la fornitura di  `imagePullSecrets`) dopo aver eliminato l'operatore con ambito namespace e prima di installare l'operatore con ambito cluster. Per un elenco completo dei parametri che possono essere aggiornati, consulta link:../trident-get-started/kubernetes-customize-deploy.html#configuration-options["opzioni di configurazione"].
+
[listing]
----

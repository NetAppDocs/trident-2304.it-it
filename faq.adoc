...
Sì, Astra Trident può essere installato offline. Vedi link:./trident-get-started/kubernetes-deploy.html["qui"].
...
Il programma di installazione di Trident crea un ClusterRole di Kubernetes, che ha accesso specifico alle risorse PersistentVolume, PersistentVolumeClaim, StorageClass e Secret del cluster Kubernetes. Vedi link:./trident-get-started/kubernetes-customize-deploy-tridentctl.html["qui"].
...
Puoi generare e modificare localmente i file manifest esatti che Astra Trident utilizza per l'installazione, se necessario. Vedi link:./trident-get-started/kubernetes-customize-deploy-tridentctl.html["qui"].
...
Ci sono diversi fattori da valutare se vuoi effettuare un downgrade. Vedi link:./trident-managing-k8s/downgrade-trident.html["la sezione sul downgrade"].

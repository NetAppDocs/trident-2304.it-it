...
----
+
++ NOTA: dopo aver eseguito questo comando, ONTAP richiede l'inserimento del certificato. Incolla il contenuto del  `k8senv.pem` file generato nel passaggio 1, quindi inserisci  `END` per completare l'installazione.
